<template>
  <div>
    ...
    <div class="row ml-2" v-for="item in fetchedFilters" :key="item.timeStamp" >
      <div>
        <b-button v-b-toggle="item.timeStamp" class="m-1">{{item.category}}</b-button>
        <b-collapse :id="item.timeStamp" v-for="subcategory in item.options" :key="subcategory.frequent_filter_count" >
          <b-card >
            <input
